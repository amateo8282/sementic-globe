...
describe("CreateMessage 유스케이스", () => {
  ...
  it("메시지를 생성하고 임베딩과 좌표를 설정하여 저장한다", async () => {
    ...
    const result = await createMessage.execute({ content: "안녕하세요" });
  ...
  it("기존 메시지의 임베딩/좌표를 PlacementService에 전달한다", async () => {
    ...
    await createMessage.execute({ content: "새 메시지" });
  ...
  it("빈 내용의 메시지는 에러를 발생시킨다", async () => {
    await expect(createMessage.execute({ content: "" })).rejects.toThrow(
      "메시지 내용은 비어있을 수 없습니다"
    );
  });
  ...
  it("280자를 초과하는 메시지는 에러를 발생시킨다", async () => {
    ...
    await expect(
      createMessage.execute({ content: longContent })
